...
- check `.claude/skills/*.md` for any that document the function/file/behavior this branch changes (grep for names touched by the diff). If a skill's claim is now stale or wrong because of this change, flag it — it should be updated in this same PR, not left as a follow-up
