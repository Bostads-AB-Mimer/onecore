...
### OneCore-owned tables

In addition to the Xpand mirror tables (read-only), this service owns a few tables that live in the same SQL Server database but are written by ONECore. Examples: `component_*`, `documents`, and the management-area tables (`onecore_cost_center`, `onecore_kvv_area`, `onecore_property_kvv_area`).
...
```sh
$ pnpm run seed:property-areas
```

The script is idempotent (upserts on natural unique keys).
