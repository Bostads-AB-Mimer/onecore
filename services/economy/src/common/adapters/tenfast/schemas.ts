...
export const TenfastInvoiceSchema = z.object({
  ...
  amount: z.number(),
  amountPaid: z.number(),
  roundingAmount: z.number().optional().default(0),
...
export const TenfastRentArticleSchema = z.object({
  ...
  accountConfigurations: z
    .array(TenfastAccountConfigurationSchema)
    .optional(),
})
...
export const TenfastInvoicesByExportedResponseSchema = z.object({
  records: z.array(
    TenfastInvoiceSchema.extend({
      ...
      recipientContactCode: data.avtal[0]?.hyresgaster[0]
        ? (data.avtal[0]?.hyresgaster[0] as { externalId?: string }).externalId
        : undefined,
