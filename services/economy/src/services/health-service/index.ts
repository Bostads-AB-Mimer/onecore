...
import { db as economyDb } from '../invoice-service/adapters/invoice-data-db-adapter'
import { db as xpandDb } from '../invoice-service/adapters/xpand-db-adapter'
...
const subsystems = [
  {
    probe: async (): Promise<SystemHealth> => {
      return await probe(
        config.health.economyDatabase.systemName,
        healthChecks,
        config.health.economyDatabase.minimumMinutesBetweenRequests,
        async () => {
          await economyDb.table('listing').limit(1)
        }
      )
    },
  },
