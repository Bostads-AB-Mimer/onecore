...
export interface StralforsAdapterConfig {
  baseUrl: string
  clientId: string
  clientSecret: string
...
export interface StralforsAdapter {
  postChannelLookup: (nationalIdentityNumbers: string[]) => Promise<
    {
      error: string | null
...
export const stralforsAdapter = (
  config: StralforsAdapterConfig
): StralforsAdapter => {
  let accessToken: string | null = null

  async function getAccessToken(): Promise<string> {
    const response = await axios.post(
      `${config.baseUrl}/v1/oidc/authorization`,
      {
  ...
  async function getChannelLookup(correlationId: string) {
    const response = await makeStralforsRequest(
      `${config.baseUrl}/rest/outputmanagement/channellookup/v1/lookup/${correlationId}`,
      {
        method: 'GET',
      }
    )
    ...
    return (
      StralforsGetChannelLookupResponseSchema.parse(response.data).results ??
      null
    )
  }

  return {
    async postChannelLookup(nationalIdentityNumbers: string[]) {
      const response = await makeStralforsRequest(
        `${config.baseUrl}/rest/outputmanagement/channellookup/v1/lookup`,
        {
          method: 'POST',
          data: {
      ...
      const pollGetChannelLookup = async (retryCount: number = 0) => {
        ...
        if (!result) {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, config.retryBackoffMs ?? 500)
          )
