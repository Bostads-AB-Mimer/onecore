...
import { ArrowLeft, Search, Filter, Plus, Download, List, Grid3X3 } from "lucide-react";
...
const generateSampleKeys = (systemCode: string): Key[] => {
  const rentalObjects = [
    'Minken 1 lgh 1001', 'Minken 1 lgh 1002', 'Minken 1 lgh 1003', 'Minken 1 lgh 1004',
    'Bävern 2 lgh 2001', 'Bävern 2 lgh 2002', 'Bävern 2 lgh 2003', 'Bävern 2 lgh 2004',
    'Ekorre 3 lgh 3001', 'Ekorre 3 lgh 3002', 'Ekorre 3 lgh 3003',
    'Räven 4 lgh 4001', 'Räven 4 lgh 4002', 'Räven 4 lgh 4003', 'Räven 4 lgh 4004',
    'Björn 5 lgh 5001', 'Björn 5 lgh 5002', 'Björn 5 lgh 5003',
    'Älg 6 lgh 6001', 'Älg 6 lgh 6002'
  ];
  ...
  rentalObjects.forEach((rentalObject, objIndex) => {
    ...
    const buildingPrefix = rentalObject.split(' ')[0].substring(0, 3).toUpperCase();
...
export default function KeySystemDetail() {
  ...
  const paginatedKeys = viewMode === "list" ? filteredAndSortedKeys.slice(
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="p-6 border rounded-lg bg-muted/50">
          <h2 className="text-lg font-semibold mb-4">Fastigheter ({properties.length})</h2>
          <div className="flex flex-wrap gap-2">
            {properties.length > 0 ? (
              ...
              <span className="text-muted-foreground">Inga fastigheter kopplade</span>
            )}
          </div>
        </div>
      </div>
                ...
                {paginatedKeys.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-muted-foreground py-8">
                      {filteredAndSortedKeys.length === 0 ? "Inga nycklar hittades" : "Inga nycklar på denna sida"}
                    </TableCell>
                  </TableRow>
                ) : (
                        ...
                        <div className="flex gap-1 ml-2">
                          {(() => {
                            ...
                            return Object.entries(counts)
                              ...
                              .map(([keyType, count]) => (
                                <Badge key={keyType} variant="outline" className="text-xs">
                                  {count} {KeyTypeLabels[keyType as keyof typeof KeyTypeLabels]}
                                </Badge>
                              ));
                          })()}
                        </div>
        ...
        {viewMode === "list" && totalPages > 1 && (
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              Visar {((currentPage - 1) * KEYS_PER_PAGE) + 1}-{Math.min(currentPage * KEYS_PER_PAGE, filteredAndSortedKeys.length)} av {filteredAndSortedKeys.length} nycklar
            </p>
            <Pagination>
              <PaginationContent>
                <PaginationItem>
