...
import type { LeaseDto, RentalPropertyResponse, Address as AddressAlias, ApiTenant } from "@/services/types";
...
export type Lease = {
  ...
  residentialArea?: { code: string; caption: string };
};
...
function toLeaseFromDto(dto: LeaseDto): Lease {
  const rentalProp = dto.rentalProperty ?? ({} as NonNullable<LeaseDto["rentalProperty"]>);
    ...
    rentInfo: dto.rentInfo
      ? {
          currentRent: {
            ...
            additionalChargeDescription: dto.rentInfo.currentRent.additionalChargeDescription,
            additionalChargeAmount: dto.rentInfo.currentRent.additionalChargeAmount,
...
function toContactFromApi(hit: ApiTenant | undefined, fallbackPnr: string) {
  ...
  const birthDate =
    (hit?.birthDate ? new Date(hit.birthDate) : new Date("1900-01-01"));
