...
export interface NullableDateFilterValue {
  hasValue: boolean | null // null = all, true = has date, false = no date
  after: string | null
  before: string | null
}

interface NullableDateFilterDropdownProps {
  label: string
  value: NullableDateFilterValue
  onChange: (value: NullableDateFilterValue) => void
  className?: string
}

export function NullableDateFilterDropdown({
  label,
  value,
  onChange,
  className,
}: NullableDateFilterDropdownProps) {
  const hasActiveFilter =
    value.hasValue !== null || value.after !== null || value.before !== null

  // Null status state
  const [nullStatus, setNullStatus] = React.useState<string>(
    value.hasValue === null ? 'all' : value.hasValue ? 'has-value' : 'no-value'
  )

  // After date state
  const [openAfter, setOpenAfter] = React.useState(false)
  const [dateAfter, setDateAfter] = React.useState<Date | undefined>(
    value.after ? new Date(value.after) : undefined
  )
  ...
  const [dateBefore, setDateBefore] = React.useState<Date | undefined>(
    value.before ? new Date(value.before) : undefined
  )
  ...
  const handleApply = () => {
    const hasValue =
      nullStatus === 'all' ? null : nullStatus === 'has-value' ? true : false
    const afterValue =
      nullStatus === 'has-value' && dateAfter
        ? dateAfter.toISOString().split('T')[0]
        : null
    const beforeValue =
      nullStatus === 'has-value' && dateBefore
      ...
      <PopoverContent className="w-80 p-4" align="start">
        <div className="space-y-4">
          <div className="flex flex-col gap-3">
            ...
            <RadioGroup value={nullStatus} onValueChange={setNullStatus}>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="all" id={`${label}-all`} />
                <Label htmlFor={`${label}-all`} className="font-normal">
                  Alla
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="has-value" id={`${label}-has`} />
                <Label htmlFor={`${label}-has`} className="font-normal">
                  Har datum
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="no-value" id={`${label}-no`} />
