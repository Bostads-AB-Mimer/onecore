...
import { AUDIENCE_LEVEL_LABELS } from '../model/labels'
  ...
  return (
    ...
    >
      ...
      <TableCell className="hidden py-2 text-muted-foreground @3xl:table-cell">
        {row.typeLabel ?? AUDIENCE_LEVEL_LABELS[row.node.level]}
      </TableCell>
...
export function ObjectRow({
  ...
  // A vacant (or not-yet-loaded) object reaches nobody, so it never shows an
  // inherited tick even when an ancestor is selected.
  const hasTenants = tenants !== undefined && tenants.length > 0
  const displayState: CheckState =
    hasTenants && !excluded ? checkState : 'unchecked'
  return (
