...
do not drift as the rest of the ONECore platform inevitably evolves.
...
- Model transformation between the _Internal ONECore Model_ and the _API Response Model_
...
- **Internal drift** (a k aPerfectly Valid and Expected):  
  Internal model changes require updating input fixtures or transform logic, but the _versioned output_ must remain identical.
...
input they expect and need to be to be updated, but they _MUST STILL PRODUCE THE SAME RESULTS_
...
But in doing so - triple-check that _nothing else_ has changed.
