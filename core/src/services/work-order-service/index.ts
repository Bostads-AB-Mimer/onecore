...
export const routes = (router: KoaRouter) => {
   ...
   *       - bearerAuth: []
   */
  router.get('/work-orders/data/:identifier', async (ctx) => {
   ...
   *       - bearerAuth: []
   */
  router.get('/work-orders/by-contact-code/:contactCode', async (ctx) => {
      ...
      try {
        ...
        if (result.ok) {
          ...
          logger.error(
            ...
            'Error getting workOrders by maintenance unit code'
          )
          ctx.status = result.statusCode || 500
          ctx.body = { error: result.err, ...metadata }
        }
      } catch (error) {
        logger.error(error, 'Error getting workOrders by maintenance unit code')
        ctx.status = 500
        ctx.body = { error: 'Internal server error', ...metadata }
        return
      }
      ...
      try {
        ...
        if (result.ok) {
          ...
          logger.error(
            ...
            'Error getting workOrders by maintenance unit code from xpand'
          )
          ctx.status = result.statusCode || 500
          ctx.body = { error: result.err, ...metadata }
        }
   ...
   *       - bearerAuth: []
   */
  router.get('/work-orders/xpand/:code', async (ctx) => {
    const metadata = generateRouteMetadata(ctx)
    try {
   ...
   *       - bearerAuth: []
   */
  router.post('/work-orders/:workOrderId/update', async (ctx) => {
    const metadata = generateRouteMetadata(ctx)
   ...
   *       - bearerAuth: []
   */
  router.post('/work-orders/:workOrderId/close', async (ctx) => {
    const metadata = generateRouteMetadata(ctx)
    const { workOrderId } = ctx.params
