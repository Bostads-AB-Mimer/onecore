...
export const routes = (router: KoaRouter) => {
   ...
   *     description: |
   *       Attach photos or documents to a specific component (e.g., installation photos, receipts).
  ...
  router.post('(.*)/components/:id/upload', async (ctx) => {
    ...
    const { fileData, fileName, contentType, componentInstanceId, caption } =
      ctx.request.body as {
        fileData?: string
        fileName?: string
        contentType?: string
    ...
    // Use URL path id if provided, otherwise fall back to body componentInstanceId
    const componentId = ctx.params.id || componentInstanceId

    const id = z.string().uuid().safeParse(componentId)
    if (!id.success) {
      ctx.status = 400
      ctx.body = { error: 'Invalid ID format', ...metadata }
      return
