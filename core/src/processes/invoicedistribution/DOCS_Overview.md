...
| [Överför Filer till Strålfors](./DOCS_Transfer_Files_to_Stralfors.md) | Hämtar nya fakturafiler från Tenfast och laddar upp dem till Strålfors via SFTP för tryck/distribution | CronJob `transfer-stralfors-files` (`mimer-onecore-operations`), vardagar 06:00 UTC — **för närvarande pausad (`suspend: true`)** |
...
- **CronJobbet är pausat.** `mimer-onecore-operations/apps/onecore/economy/transferstralforsfilescronjob.yaml` har `suspend: true`. Värt att bekräfta med teamet om det är avsiktligt (t.ex. under uppsättning/väntan på godkännande från Strålfors) eller en glömd paus, innan den här dokumentationen tas som sanning om vad som faktiskt körs i produktion.
